...
boolean axGraph =true,ayGraph=true,azGraph=true,gxGraph=true,gyGraph=true,gzGraph=true,baroGraph=false,headGraph=false, magxGraph =false,magyGraph=false,magzGraph=false;
...
Button buttonREAD,buttonWRITE,buttonCALIBRATE_ACC_GYRO,buttonSTART,buttonSTOP;

Button buttonI2cAcc;
Button buttonI2cAccActive;
...
void setup() {
  ...
  commListbox = controlP5.addListBox("portComList",5,65 + 30,110,240); // make a listbox and populate it with the available comm ports
  ...
  controlP5.addTextlabel("txtlblPicopter","Picopter Edition",5,42);
  
  // text label for which comm port selected
  txtlblWhichcom = controlP5.addTextlabel("txtlblWhichcom","No Port Selected",5,42 + 30); // textlabel(name,text,x,y)
  ...
  */
  ...
  buttonCALIBRATE_ACC_GYRO = controlP5.addButton("CALIB",1,xParam+210,yParam+260,70,16);buttonCALIBRATE_ACC_GYRO.setColorBackground(red_);
...
void draw() {
  ...
  background(80);
  textFont(font15);
  text("multiwii.com",0,16);text("v1.9", 0, 32);
  ...
  time1=millis();
  if (init_com==1 && (time1-time2)>100 && graph_on==1) {
    try {
      g_serial.write('M');
    }
    catch (Exception ex) {
      javax.swing.JOptionPane.showMessageDialog(null, "Serial Port Failure, Exiting...");
      exit();
    }
    time2=time1;
  }
...
public void bSTART() {
  ...
  buttonREAD.setColorBackground(green_); buttonCALIBRATE_ACC_GYRO.setColorBackground(green_);
  try {
  g_serial.clear();
  }
   catch (Exception ex) {
...
public void WRITE() {
  ...
  int p = 0;
   try {
     g_serial.write('W'); //0 write to Eeprom @ arduino //1
   }
   catch (Exception ex) {
     javax.swing.JOptionPane.showMessageDialog(null, "Serial Port Failure, Exiting...");
     exit();
   ...
   try {
     g_serial.write(char(p));
     for(int i =0;i<p;i++) {
       g_serial.write(char(s[i]));
       serChkSum += s[i];
     }
     g_serial.write(char(serChkSum));
   }
   catch (Exception ex) {
     javax.swing.JOptionPane.showMessageDialog(null, "Serial Port Failure, Exiting...");
     exit();
