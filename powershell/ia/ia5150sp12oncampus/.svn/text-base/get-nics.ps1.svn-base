...
$outputFile = "Z:\.WIN_PROFILE\Desktop\ia5150_dhcp_spring_oncampus.txt"
$ipPrefix = "10.0.11"
$numPods = 12 # Number of Pods to create rules for
$numVms = 4 # Number of VMs per Pod
$ipPodPostfix = 10 # Starting IP to go up from
$ipPodPostfixSkip = 10 # Number of IPs to jump per pod
$natPodStart = 20001 # Base NAT port
...
for ($i=0; $i -lt $vmTmplName.length; $i++) {
	...
	$vmTemplateName = $vmClassName + "-" + $vmClassSem + "-oncampus-" + $vmTmplName[$i]
...
$stream = [System.IO.StreamWriter] "Z:\.WIN_PROFILE\Desktop\ia5150_nat_oncampus.txt"
$index = 0
While ($index -lt $numPods) {
	...
	$stream.WriteLine("		<target>$ipPrefix.$natTarget</target>")
